...
(ref:figure1) Location of the boundaries of the mainland inlet localities, one exploratory site and the five spatial areas (S~1~-S~5~) of the stratified random survey design. The three depths strata (RD~1~-RD~3~) are colour-coded and nested within each of the five spatial strata. 

```{r figure1, fig.cap='(ref:figure1)', out.width = "410px", fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(basepath,'figure1.png',sep='') 
      knitr::include_graphics(img)
```
\clearpage

(ref:figure2) Start locations of survey sets in `r yr` conducted in the stratified random survey areas S~1~ through S~5~ (red markers) and in the exploratory area (purple markers).  The camera sets are represented by yellow markers.
...
```{r figure2, fig.cap='(ref:figure2)', out.width = "480px", out.height = "381px", fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(basepath,'figure2.png',sep='') 
      knitr::include_graphics(img)
```
\clearpage

(ref:figure3)  Image of the F/V `r boat`  used for the `r yr` sablefish research and assessment survey. Photo credit:    .

```{r figure3, fig.cap='(ref:figure3)', out.width = "480px", out.height = "360px", echo=FALSE, fig.align="center", warning=FALSE}
...
```{r figure4, fig.cap='(ref:figure4)', out.width = "370px", out.height = "610px", echo=FALSE, fig.align="center", warning=FALSE}
...
```{r figure5, fig.cap='(ref:figure5)',echo=FALSE, out.width = "400px",  fig.align = "center", warning=FALSE}
    ...
    png(paste(basepath,'figure5.png',sep=''), width = 475, height = 684)  # write png to folder 
    ...
    img    <-  paste(basepath,'figure5.png',sep="")   # retrieve png 
               knitr::include_graphics(img)
```
\clearpage
...
``` 
\clearpage
(ref:figure11) Sablefish fork length (L in cm) vs weight (W in kg) for females (A) and males (B) for the `r yr` survey.

```{r figure11, fig.cap='(ref:figure11)',results='asis', out.width = "480px", out.height = "282px", fig.align="center"}

    # Sablefish.dbo.procR_Survey_Sablefish_LenWt_shiny queries table GFBIO_SABLEBIO_VW for survey length data	
    png(paste(basepath,'figure11.png',sep=''), width = 700, height = 412) # -- starts writing a png to figure folder
    ...
    #write.table(lenWTdat1, file = paste(path,"figure11.csv",sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")    
    lenWTdat1 <- read.csv(paste(path,'figure11.csv',sep=''),header=T) # read from csv
...
(ref:figure13) Coplot of average depth (m) vs average temperature ($^\circ$C) for a given 1-degree latitude range (blue bands) for `r yr`.  The number of fishing sets deployed with a SBE 39 recorder are represented by n.

```{r figure13, fig.cap='(ref:figure13)',results='asis', out.width = "500px",out.height = "335px", fig.align="center"}
...
```
\clearpage
(ref:figure14) Vertical density ridgeplots of mean temperatures per year as reported by set from the Sea-bird SBE 39 loggers on traps at three depth intervals, RD~1~ = shallow (100-450 m), RD~2~ = mid (450-850 m), RD~3~ = deep (850-1400 m). Lines indicate the 2.5% and 97.5% tails.

```{r figure14, fig.cap='(ref:figure14)', results='asis', out.width = "440px",out.height = "540px", fig.align = "center"}
...
(ref:figure15) Location of the epicentre of a 7.7 magnitude earthquake on Saturday October 27, 2012 at 8:04 p.m.  The accelerometer on the ocean floor captured the earthquake event (inset).

```{r figure15, fig.cap='(ref:figure15)', out.width = "450px", out.height = "582px",echo=FALSE, fig.align="center", warning=FALSE}
      img  <- paste(basepath,'figure15.png',sep="") 
      knitr::include_graphics(img)
```
\clearpage
